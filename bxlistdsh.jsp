...
<script type="text/javascript">
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/bxmanadsh.action?index="+totlePage;
	            submit();
	        }
		}
		...
		function changesearch()
		{
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/bxmanadsh.action?index=1";
	            submit();
	        }
		}
      ...
      <tr>
       ...
       <%
         ...
         <td><%=bean.getTimes() %></td>
          ...
          <a class="button border-main" href="<%=path %>/bxsh.action?id=<%=bean.getId()%>&types=1"><span class="icon-edit"></span> Passed</a>
           <a class="button border-red" href="<%=path %>/bxsh.action?id=<%=bean.getId()%>&types=2" >
           <span class="icon-trash-o"></span>Not Passed</a> </div></td>
        </tr>
        <%} %>
      <tr>
